<template>
    ...
    <div class="hd log url">
        <h2>{{title}}</h2>
        <router-link :to="{name:'morelist',params:{type:type,title:title}}">更多</router-link>
    </div>
    <div class="container">
        <div class="gallery">
            <div class="scroller">
                <!--这里的for是在数组中的 -->
                <router-link tag="li" :to="{name:'musicplay',params:{song_id:item.song_id}} " class="card url" v-for="(item,index) in todayRecommend" :key="index">
                    <div class="album">
